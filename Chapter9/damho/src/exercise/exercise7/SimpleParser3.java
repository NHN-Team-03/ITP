...
import exercise.exercise6.TextIO;
...
public class SimpleParser3 {
    ...
    private static class BinOpNode extends ExpNode {
        ...
        ExpNode derivative() {
            ...
            switch (op) {
                case '+':
                case '-':
                    return new BinOpNode(op, left.derivative(), right.derivative());
                case '*':
                    return new BinOpNode('+',
                            new BinOpNode('*', left, right.derivative()),
                            new BinOpNode('*', right, left.derivative()));
                case '/':
                    BinOpNode rightMultiplyDerivativedLeft = new BinOpNode('*', right, left.derivative());
                    BinOpNode leftMultiplyDerivativedRight = new BinOpNode('*', left, right.derivative());
                    BinOpNode numerator = new BinOpNode('-', rightMultiplyDerivativedLeft,
                            leftMultiplyDerivativedRight);
                    BinOpNode denominator = new BinOpNode('*', right, right);
    ...
    /**
     * x와 같은 variable을 표현하기 위한 expression node.
     */
    ...
    private static ExpNode factorTree() throws ParseError {
        ...
        if (Character.isDigit(ch)) {
            ...
            char character = TextIO.getAnyChar();
            return new VariableNode(character);
        } else if (ch == '(') {
