package Chapter9.jaehun.src.exercise.exercise7;

import Chapter9.jaehun.src.exercise.textio.*;
...
public class Exercise7 {
    ...
    private static class BinOpNode extends ExpNode {
        ...
        @Override
        ExpNode derivative() {
            ExpNode leftDerivative = left.derivative();
            ExpNode rightDerivative = right.derivative();

            switch (op) {
                case '+':
                    return new BinOpNode('+', leftDerivative, rightDerivative);
                case '-':
                    return new BinOpNode('-', leftDerivative, rightDerivative);
                case '*':
                    return new BinOpNode('+', new BinOpNode('*', left, rightDerivative),
                            new BinOpNode('*', right, leftDerivative));
                case '/':
                    return new BinOpNode('/', new BinOpNode('-', new BinOpNode('*', right, leftDerivative),
                            new BinOpNode('*', left, rightDerivative)), new BinOpNode('*', right, right));
                default:
                    return null;
            }
        }
    ...
    private static class VariableNode extends ExpNode {
        ...
        @Override
        void printStackCommands() {
            System.out.println("  Push " + xVariable);
        }
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            try {
                ...
                TextIO.getln();
                for (int i = 0; i < 4; i++) {
                    System.out.println("\nValue is " + i + " : " + exp.value(i));
                }
                ExpNode derivativeExp = exp.derivative();
                for(int i = 0; i < 4; i++) {
                    System.out.println("\nValue is " + i + " : " + derivativeExp.value(i));
                }
