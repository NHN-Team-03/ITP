...
* 3-2. 소스 코드를 올려야 할 경우 자신의 폴더 하위에 `src` 폴더를 생성합니다.
* 3-3. 연습문제인 경우 src 폴더 하위에 `Exercise` 폴더를 생성합니다.
* 3-4. 예제인 경우 src 폴더 하위에 `Example` 폴더를 생성하고 이 폴더에 정리합니다. (파일명: 자유)
* 3-5. 연습문제 번호에 맞게 `Exercise-number.java` 파일 이름을 정합니다.
* 3-6. `*Chapter1/seungjo/src/Exercise/Exercise1.java*` 와 같은 경로가 되도록 합니다.
